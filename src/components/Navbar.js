...
const Navbar = () => {
  ...
  return (
    <nav className=' w-full h-20 p-2 flex justify-around shadow-lg text-white'>
      ...
      <ul className='hidden md:flex lg:-mt-11 md:mt-0 font-montserrat font-semibold'>
        {NAV_ITEMS.map(item => (
          <li
            key={item.id}
        ...
        <button>
          <img src='https://storage.googleapis.com/papercycle_prod/Ellipse%2014.svg' height={50} width={60} alt="Ellipse" className='lg:mt-10 ' />
        </button>
      </ul>
      ...
      >
        <li className='p-4 border-b rounded-xl duration-300 text-black font-montserrat font-semibold cursor-pointer border-gray-600'>
